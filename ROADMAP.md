...
M26: 永久世界 ✅ (2026-07-08) — 角色死亡保留世界，新角色继承
...
M27: 跨局遗产 ✅ (2026-07-08)
  - systems/legacy_system.py: 遗产点数计算/商店/增益应用
  - data/legacy.json: 跨局数据（点数/解锁配方/前世记录）
  - ui/states/legacy_state.py: 遗产商店界面
  - 8种增益：石剑/石镐/食物/挖掘经验/战斗经验/生命强化/铁器时代/木箱
  - 点数来源：基础+技能等级+深度+狩猎数
  - 死后自动记录前世，配方自动传承

M28: 自定义键位 ✅ (2026-07-08)
  - data/keybinds.json: 外部键位配置（20+ 可配置按键）
  - systems/keybind.py: 加载/解析/重载模块
  - config.py: 启动时覆盖默认常量
  - 支持 curses 常量名（KEY_UP/KEY_ENTER）和单字符
  - 首次运行自动生成默认配置文件
  - 按 ? 运行时重载键位

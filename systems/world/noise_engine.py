...
def _hash2d(x: int, y: int, seed: int) -> int:
    """Compute a deterministic pseudo-random hash from 2D coordinates and a seed."""
...
def _hash_uniform(x: int, y: int, seed: int) -> float:
    """Compute a deterministic pseudo-random value uniformly distributed in [0, 1]."""
    return (_hash2d(x, y, seed) & 0x7fffffff) / 0x7fffffff


def _smooth_noise(x: int, y: int, seed: int) -> float:
    """Compute smoothed noise at an integer coordinate using neighbor averaging."""
...
def _interpolated_noise(x: float, y: float, seed: int) -> float:
    """Compute interpolated Perlin-style noise at a floating-point coordinate."""
    ix, iy = int(x), int(y)
    fx, fy = x - ix, y - iy

    def _fade(t):
        """Apply the Perlin fade curve to smooth an interpolation factor."""

...
1. **合成门窗功能缺失**：建造系统无法合成门/窗，未排查是crafting_state.py还是配方数据缺失。
2. **`place_tile`字段名实无实**：`tile_props.py::_build_placeable_props()`遍历items.json时直接用key当tile名，从未读取`place_tile`字段值。目前能正常工作纯属"字段值与key恰好相同"的巧合。风险：以后配好新物品若两者不一致，会静默出现属性查不到（不崩溃，只是数值/显示不对，难排查）。修复思路二选一：改代码真正读取该字段，或删除该字段避免误导。
